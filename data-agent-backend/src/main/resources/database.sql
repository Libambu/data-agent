-- 数据库与扩展
-- 第一次使用前手动执行：CREATE DATABASE data_agent;
-- 然后切换到 data_agent 库再执行下面的 SQL。

-- pgvector 扩展，给 Spring AI 的 vector_store 用（Spring AI 启动时也会自动创建，这里写出来便于手动初始化）
CREATE EXTENSION IF NOT EXISTS vector;

-- 1. GlossaryKnowledge 业务名词知识表
CREATE TABLE IF NOT EXISTS glossary_knowledge
(
    id          UUID PRIMARY KEY,
    database_id VARCHAR(255) NOT NULL,
    term        VARCHAR(255) NOT NULL,
    description TEXT         NOT NULL,
    synonyms    VARCHAR(255)
);

-- 2. QuestionKnowledge 历史问答知识表
CREATE TABLE IF NOT EXISTS question_knowledge
(
    id          UUID PRIMARY KEY,
    database_id VARCHAR(255) NOT NULL,
    question    TEXT         NOT NULL,
    answer      TEXT         NOT NULL
);

-- 3. DbTable 业务库表元数据
CREATE TABLE IF NOT EXISTS db_table
(
    id          UUID PRIMARY KEY,
    name        VARCHAR(255) NOT NULL,
    description TEXT         NOT NULL,
    database_id VARCHAR(255) NOT NULL,
    -- DbTable 的 @Key (name + databaseId) 唯一约束
    CONSTRAINT uq_db_table_name_database UNIQUE (database_id, name)
);

-- 4. DbColumn 业务库列元数据
CREATE TABLE IF NOT EXISTS db_column
(
    id             UUID PRIMARY KEY,
    name           VARCHAR(255) NOT NULL,
    type           VARCHAR(255) NOT NULL,
    description    TEXT         NOT NULL,
    is_primary_key BOOLEAN      NOT NULL,
    table_id       UUID         NOT NULL,
    CONSTRAINT fk_db_column_table_id FOREIGN KEY (table_id) REFERENCES db_table (id) ON DELETE CASCADE,
    -- DbColumn 的 @Key (name + dbTable) 唯一约束
    CONSTRAINT uq_db_column_table_name UNIQUE (table_id, name)
);

-- 5. DbForeignKey 外键关系表
CREATE TABLE IF NOT EXISTS db_foreign_key
(
    id               UUID PRIMARY KEY,
    source_column_id UUID NOT NULL,
    target_column_id UUID NOT NULL,
    CONSTRAINT fk_db_fk_source_column FOREIGN KEY (source_column_id) REFERENCES db_column (id) ON DELETE CASCADE,
    CONSTRAINT fk_db_fk_target_column FOREIGN KEY (target_column_id) REFERENCES db_column (id) ON DELETE CASCADE,
    -- DbForeignKey 的 @Key (sourceColumn + targetColumn) 唯一约束
    CONSTRAINT uq_db_foreign_key_source_target UNIQUE (source_column_id, target_column_id)
);
